...
class PRN:
    ...
    def process3DFile(self,image,recs,depth=False, pose = False,name='default'):
        ...
        [h, w, c] = image.shape
        pos,vertices,save_color = self.get_3D(image,recs)
        kpt_3d = self.get_landmarks(pos)
        ...
        if pose:
            afine, pose = estimate_pose(vertices,self.canonical_vertices)
    ...
    def sorce_pose(self, image,recs):
