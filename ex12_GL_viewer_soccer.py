...
def event_key(window, key, scancode, action, mods):
    ...
    if action == glfw.PRESS:
        ...
        if key == ord('A'): R.rotate_view((0,+d,0))
        if key == ord('D'): R.rotate_view((0,-d,0))
...
def event_position(window, xpos, ypos):
    ...
    if R.on_rotate == True:
        ...
        R.rotate_view((delta_angle[1], delta_angle[0],0))

    return
# ----------------------------------------------------------------------------------------------------------------------
def event_scroll(window, xoffset, yoffset):
    if R.projection_type=='P':
        if yoffset>0:R.scale_projection(2)
        else        :R.scale_projection(1.0 / 2)
    return
# ----------------------------------------------------------------------------------------------------------------------
